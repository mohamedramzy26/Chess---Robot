...
chess.SQUARE_NAMES.index("h8") print(chess.piece_name(board.piece_type_at(chess.A7)))
...
def showCheck(board): if board.is_check(): print("CHECK!!")

def sequenceGenerator(uciMove, board): ''' INPUT: uciMove -> Move in UCI format to analyze it. board -> Game state, this is a chess.Board() object. OUTPUT: result -> Is empty if a valid move is not detected, if it is detected then it is a dictionary data structure with two values: "seq": String with a sequence of chess coordinates to guide th robot movement. "type": String with type of movement, Castling-Promotion-Move-Capture-Passant. ''' result = {} move = chess.Move.from_uci(uciMove) first = uciMove[:2] second = uciMove[2:4] last = uciMove[-1] graveyard = "k0"
...
def moveAnalysis (squares, board): ''' INPUT: squares -> Squares detected by the computer vision algorithm. board -> State of game, this is a chess.Board() object. OUTPUT: result -> Is empty if a valid move is not detected, or on the contrary, a dictionary data structure with two values: "move": String with the valid movement in UCI format, for example "e2e4". "type": String with type of movement, Castling-Promotion-Move-Capture-Passant.
